...
const floatingShutterBtn = document.getElementById("floating-shutter-btn") as HTMLButtonElement;
...
folderBtn.addEventListener("click", async () => {
  try {
    ...
    floatingShutterBtn.disabled = false;
...
async function capturePhoto() {
  ...
  floatingShutterBtn.disabled = true;
...
floatingShutterBtn.addEventListener("click", capturePhoto);
...
fullscreenBtn.addEventListener("click", () => {
  if (document.fullscreenElement) {
    ...
    previewWrap.requestFullscreen().catch((err) => {
      log(`Gagal masuk fullscreen: ${(err as Error).message}`, "error");
    });
  }
});

document.addEventListener("fullscreenchange", () => {
  const isFullscreen = document.fullscreenElement === previewWrap;
  fullscreenBtn.textContent = isFullscreen ? "✕" : "⛶";
  fullscreenBtn.title = isFullscreen ? "Keluar Fullscreen" : "Perbesar / Fullscreen";
});
